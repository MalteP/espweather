 ...
 * <port> is optional
 */
bool ICACHE_FLASH_ATTR http_post(const char * url, const char * post_data, const char * headers, http_callback user_callback)
{
	...
	if (is_http)
		...
		#else
		os_printf("HTTPS support disabled\n");
		return false;
		#endif
	} else {
...
bool ICACHE_FLASH_ATTR http_get(const char * url, const char * headers, http_callback user_callback)
{
